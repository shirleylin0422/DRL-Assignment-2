...
def td_learning(env, approximator, start_episode=0, num_episodes=80000, alpha=0.01, gamma=0.99, epsilon=0.1):
    final_scores = []
    success_flags = []
    ...
    for episode in tqdm(range(start_episode, num_episodes), desc="Training Episodes"):
        ...
        if (episode + 1) % 1000 == 0:
                filename = f"train_file/OTD_learning_15W_lr02/td_table_episode_{episode+1}.pkl"
...
def main():
    ...
    with open("train_file/OTD_learning_15W_lr02/td_table_episode_150000.pkl", "rb") as f:
        loaded_weights = pickle.load(f)
    approximator.weights = convert_weights(loaded_weights, approximator.patterns)
    
    env = Game2048Env()
    final_scores = td_learning(env, approximator, start_episode=150000, num_episodes=300000, alpha=0.1, gamma=1)
    window = 100
